...
public interface RecordRepository extends CrudRepository<Record, Integer> {
    ...
    public void updateTnwarehousedTime(Integer id, Time time);

    @Modifying
    @Transactional
    @Query("update Record record set record.exwarehousingTime = ?2 where record.recordId = ?1")
    public void updateExwarehousingTime(Integer id, Time time);

    @Modifying
    @Transactional
    @Query("update Record record set record.exwarehousedTime = ?2 where record.recordId = ?1")
    public void updateExwarehousedTime(Integer id, Time time);
}
